...
public class Checkers extends JPanel implements ActionListener, ItemListener, MouseMotionListener, MouseListener {
    ...
    JTextArea msg=new JTextArea("Старт новой игры... желтые ходят первые...");
    ...
    JButton nwB=new JButton("Новая игра");
    JButton unB=new JButton("Отмена");
    //JButton hlpB=new JButton(hlp);
   // JButton snB=new JButton(snp);

    ButtonGroup players = new ButtonGroup();
    JRadioButton p1 = new JRadioButton("Игра с ИИ", true);
    JRadioButton p2 = new JRadioButton("2-Игрока", false);
   ...
   // Help hp=new Help();
    ...
    JLabel mode=new JLabel("Режим");
    JLabel col=new JLabel("Colour");
    JLabel diff=new JLabel("Сложность");
    JLabel rp=new JLabel();
    JLabel rpt=new JLabel("Противник");
    JLabel bpt=new JLabel("Вы");
    JLabel bp=new JLabel();
    JLabel rk=new JLabel();
    JLabel rkt=new JLabel("Дамка противника");
    JLabel bkt=new JLabel("Ваша дамка");
    ...
    private void setupGUI(){
       ...
       // hlpB.setBounds(415,10,25,25);
        ...
        level.addItem("Новичек");
        level.addItem("Любитель");
        level.addItem("Средний");
        level.addItem("Ниче так");
        level.addItem("Профи");
    ...
    public void actionPerformed(ActionEvent e) {
        if(e.getActionCommand().equalsIgnoreCase("Игра с ИИ")){
        ...
        if(e.getActionCommand().equalsIgnoreCase("2-Игрока")){
           // new PlaySound("src//sounds//option.wav").start();
        ...
        if(e.getActionCommand().equalsIgnoreCase("Новая игра")){
          //  new PlaySound("src//sounds//button.wav").start();
            newGame();
        }
        if(e.getActionCommand().equalsIgnoreCase("Отмена") && undoCount>3){
           // new PlaySound("src//sounds//button.wav").start();
            undo();
        }
//        if(e.getSource()==hlpB){
...
//        }
    ...
    public void newGame()	{
		...
		board[0][5]=yellowKing;

        toMove = yellowNormal;

        for(int i=0;i<8;i++){
            System.arraycopy(board[i],0,preBoard1[i],0,8);                       //для отмены
    ...
    public void drawCheckers(){                   //Рисует шашки на доске
       g=getGraphics();
    ...
    public void undo(){            //функция отмены
        undoCount=1;
        for(int i=0;i<8;i++){
            System.arraycopy(preBoard3[i],0,board[i],0,8);              //копирует предыдущую доску
        }
        toMove=preToMove3;
        drawCheckers();
        update(g);
    ...
    public void mousePressed(MouseEvent e) {
		...
		(board[square[0]][square[1]] == Checkers.yellowNormal || board[square[0]][square[1]] == Checkers.yellowKing))
		{
			...
			// мы не хотим потерять неполную информацию о движении
			// только устанавливают новые стартовые переменные, если! неполные
			if (!incomplete)
			{
				highlight = true;
				startX = square[0];
    ...
    private void showStatus() {       //Печать сообщений в статус бар
        if (this.toMove == redNormal){
            msg.setText("Ход красных");
        }
        else{
            msg.setText("Ход желтых");
        }

        if (loser == redNormal && won==0){
            msg.setText("Желтые выйграли!");
            try {
                Thread.sleep(150);
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
            new GameWin("Желтые",this.getLocationOnScreen());
            won=1;
            undoCount=0;
            newGame();
        }
        else if (loser == yellowNormal && won==0){
            msg.setText("Красные выйграли!");
            try {
                Thread.sleep(150);
            } catch (InterruptedException e) {
                e.printStackTrace();
            }            
            new GameWin("Красные",this.getLocationOnScreen());
            won=1;
            undoCount=0;
            newGame();            
        }
